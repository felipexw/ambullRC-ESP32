...
the existing Bluetooth control connection — no new connection type, per spec Clarifications. See
`tone-output-hardware-contract.md` for what the car does when a trigger is honored.
...
|---|---|
| `HORN` | `ParseResult::Ok` |
| `ENGINE`, `SIREN` | `ParseResult::Malformed` — both effects were removed (fall through to the existing parsers, unchanged) |
...
- No wire command exists for any other sound effect; the existing drive-command wire format
  (`UP`/`DOWN`/`STOP`/etc.) is not changed by this contract.
